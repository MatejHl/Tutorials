# Ref:    https://github.com/tensorflow/transform/issues/49
#         https://github.com/galderic/transform/blob/master/examples/mpg_example.py

def fn():
    dataset = (tf.data.TextLineDataset(file_path, buffer_size=64))
    dataset = dataset.apply(tf.contrib.data.map_and_batch(decode_csv,   batch_size, 
    ...
    _, transformed_features =   saved_transform_io.partially_apply_saved_transform(
        bla, #...path to output of tf.Transform...,
        raw_features)
    
    transformed_labels = transformed_features.pop(
        bla) # ...key of     transformed label...
