...
def loss_discriminator(Discriminator_model, batch_original_x, batch_gen_x):
    ...
    """
    loss_original = tf.reduce_mean(tf.square(tf.subtract(Discriminator_model(batch_original_x), tf.ones(batch_original_x.shape))))
    loss_gen = tf.reduce_mean(tf.square(tf.subtract(Discriminator_model(batch_gen_x), tf.zeros(batch_gen_x.shape))))
...
class Generator(tf.keras.Model):
    ...
    def __init__(self, noise_shape, img_shape):
        ...
        self.dense_input = tf.keras.layer.Dense(input_shape = noise_shape,
                                                units = 256,
                                                activation = tf.nn.leaky_relu,
                                                use_bias = True)

        self.batch_norm_1 = tf.keras.layers.BatchNormalization(momentum = 0.8)
        self.dense_1 = tf.keras.layer.Dense(units = 512,
                                            activation = tf.nn.leaky_relu,
                                            use_bias = True)

        self.batch_norm_2 = tf.keras.layers.BatchNormalization(momentum = 0.8)
        self.dense_2 = tf.keras.layer.Dense(units = 1024,
                                            activation = tf.nn.leaky_relu,
                                            use_bias = True)
        
        self.batch_norm_3 = tf.keras.layers.BatchNormalization(momentum = 0.8)
        self.dense_3 = tf.keras.layer.Dense(units = tf.math.reduce_prod(img_shape),
    ...
    def call(self, input_noise):
        ...
        b_3 = self.batch_norm(d_2)
        d_3 = self.dense_3(b_3)
        to_img = self.reshape_to_img(d_3)
        return to_img


def loss_generator(Generator_model, batch_x, Discriminator_model):
    """
    Generator loss: \sum_{x\sim N(0,1)} (g(f(x)) - 1)^2

    Parameters:
    -----------

    """
    loss = tf.reduce_mean(tf.square(tf.subtract(Discriminator_model(Generator_model(batch_x)), tf.ones(batch_x.shape))))
    return loss

def train_generator(loss, Generator_model, opt, batch_x, Discriminator_model):
  with tf.GradientTape() as tape:
    gradients = tape.gradient(loss(Generator_model, batch_x, Discriminator_model), Generator_model.trainable_variables)
