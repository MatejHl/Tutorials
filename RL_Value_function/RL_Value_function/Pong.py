...
def conv_net(data, out_dim, dropout_rate, frame_window):
    ...
    with tf.compat.v1.name_scope("conv_input"):
        W = tf.compat.v1.get_variable('c_in_W', shape=(3,3,frame_window,16),
        ...
        b = tf.compat.v1.get_variable('c_in_b', shape=(16), initializer=tf.compat.v1.zeros_initializer())
        conv_input = conv2d(data, W, b) # [80,80,16]
    with tf.compat.v1.name_scope("conv_1"):
        W = tf.compat.v1.get_variable('c_1_W', shape=(3,3,16,16),
            initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
        b = tf.compat.v1.get_variable('c_1_b', shape=(16), initializer=tf.compat.v1.zeros_initializer())
        conv_1 = conv2d(conv_input, W, b) # [80,80,16]
        pool_1 = tf.nn.max_pool2d(input=conv_1, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME') # [40,40,16]
    with tf.compat.v1.name_scope("conv_2"):
        W = tf.compat.v1.get_variable('c_2_W', shape=(3,3,16,32),
            initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
        b = tf.compat.v1.get_variable('c_2_b', shape=(32), initializer=tf.compat.v1.zeros_initializer())
        conv_2 = conv2d(pool_1, W, b) # [40,40,32]
    with tf.compat.v1.name_scope("conv_3"):
        W = tf.compat.v1.get_variable('c_3_W', shape=(3,3,32,32),
            initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
        b = tf.compat.v1.get_variable('c_3_b', shape=(32), initializer=tf.compat.v1.zeros_initializer())
    ...
    with tf.compat.v1.name_scope("conv_dense_1"):
        W = tf.compat.v1.get_variable('c_d1_W', shape=(20*20*32,128), 
            initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
        b = tf.compat.v1.get_variable('c_d1_b', shape=(128), initializer=tf.compat.v1.zeros_initializer())
        # Flatten
    ...
    with tf.compat.v1.name_scope("conv_dense_output"):
        W = tf.compat.v1.get_variable('c_out_W', shape=(128, out_dim), 
            initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
...
if __name__ == '__main__':
    ...
    eps = 0.2 # 1/eps in theory, esp-greedy policy
    eps_decay = 2.0/3.0
    eps_decay_freq = 3000
    ...
    with tf.compat.v1.Session(graph=graph_Q, config=config) as sess_Q:
        if restore:
        ...
        with tf.compat.v1.Session(graph=graph_fixed_Q) as sess_fixed_Q:
            for epoch in range(n_epochs):
                ...
                for i in range(batch_size):
                    feed_dict = {state: [next_states], dropout_rate: 0.0, learning_rate: learn_rate, y_values: np.zeros((1, possible_actions_n))}
                    Q_future_state_a = sess_Q.run(Q_value, feed_dict = feed_dict)
                    argmax_a = np.argmax(Q_future_state_a, axis = 1)
